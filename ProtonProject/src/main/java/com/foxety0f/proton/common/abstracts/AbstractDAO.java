...
public abstract class AbstractDAO {
	...
	return "alter table " + tableName + " add constraint " + tableName.toUpperCase() + "_PK primary key ("
		+ columnName + ")";
    }

    public Integer logger(ProtonModules module, Map<String, ?> params, ProtonEssences essence,
	    NamedParameterJdbcTemplate src) {
	String logTable = loggerMapper().get(module);
	Integer seqNum = querySource.queryForObject("select nextval('public.\"LoggerNumSequence\"')", EMPTY_PARAMS,
		Integer.class);
	...
	String sql = "insert into " + logTable + " values(" + seqNum + ", '" + module.moduleName() + "', '"
		+ essence.getString() + "', CURRENT_TIMESTAMP)";
	...
	querySource.update("insert into z_logger_user (id, username) " + " values(:seq, :username)", map1);
    }
    ...
    public void loggerWithUser(ProtonModules thisModule, Map<String, ?> map, ProtonEssences essence,
	    UserDetailsProton user) {
	...
	map.put("key", key);
	map.put("value", value);
	return src.queryForObject("select count(*) from " + tableName + " where :key = :value", map, Integer.class) == 0
		? false
		: true;
    }
